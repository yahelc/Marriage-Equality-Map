...
function marriageToggle(status){
  ...
  window.onload = function () {
	...
	if(hasEquality.indexOf(state)>-1){
		usRaphael[state].color = usRaphael[state].originalColor = colors.gay;
		usRaphael[state].marriage = 	usRaphael[state].originalMarriage = "equality";
	}
	else if(constitutionallyBanned.indexOf(state)>-1){
			usRaphael[state].color = usRaphael[state].originalColor =  colors.banned;
		usRaphael[state].marriage = 	usRaphael[state].originalMarriage = "ban";
	}
	else if(legislativelyBanned.indexOf(state)>-1){
			usRaphael[state].color = usRaphael[state].originalColor =  colors.law;
			usRaphael[state].marriage = 	usRaphael[state].originalMarriage = "law";
		
	}
	else{
			usRaphael[state].color = usRaphael[state].originalColor =  colors.partial;
			usRaphael[state].marriage = 	usRaphael[state].originalMarriage = "partial";
			
	}
	  usRaphael[state].population = population[state];
      usRaphael[state].animate({fill: usRaphael[state].color}, 500);
	  usRaphael[state].state = state;
	  usRaphael[state].node.setAttribute("id", state);
